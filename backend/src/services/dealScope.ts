...
import type { IDeal } from "../models/Deal.js";
...
export async function buildDealScopeFilter(
  requester: Requester
...
export async function canAccessDeal(requester: Requester, deal: IDeal): Promise<boolean> {
  if (requester.role === "admin") {
    return true;
  }
  const assignedToId = deal.assignedTo.toString();
